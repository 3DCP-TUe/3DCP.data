...
% Plot data
x = data.pressure_seal;
y = data.pressure_direct;
...
text(0.25, 0.70, eqn_text, 'Units', 'normalized', 'Interpreter', 'latex', 'FontSize', 18, 'HorizontalAlignment', 'center');
text(0.25, 0.60, r2_text, 'Units', 'normalized', 'Interpreter', 'latex', 'FontSize', 18, 'HorizontalAlignment', 'center');

% Layout
xlim([0 30])
ylim([0 30])
xlabel('Pressure seal [bar]', 'interpreter', 'latex');
ylabel('Direct pressure [bar]', 'interpreter', 'latex');

% Save figure
cd(filepath);
save_figure(fig, 'pressure-fit')
